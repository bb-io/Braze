...
public class PollingList(InvocationContext invocationContext) : Invocable(invocationContext)
{
    ...
    public async Task<PollingEventResponse<DateMemory, PollingCanvasResponse>> OnCanvasUpdated(
        PollingEventRequest<DateMemory> request)
    {
        try
        {
            return await HandleCanvasUpdatedPolling(request);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnCanvasUpdated: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }

    [PollingEvent("On campaign updated" , Description = "Triggers when a campaign is updated")]
    public async Task<PollingEventResponse<DateMemory, PollingCampaignResponse>> OnCampaignUpdated(PollingEventRequest<DateMemory> request)
    {
        try
        {
            return await HandleCampaignUpdatedPolling(request);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnCampaignUpdated: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }

    [PollingEvent("On campaign message tag added", Description = "Triggers when a campaign message tag is added")]
    public async Task<PollingEventResponse<DateMemory, PollingCampaignResponse>> OnCampaignTagAdded(
        PollingEventRequest<DateMemory> request,
        [PollingEventParameter] PollingCampaignRequest input)
    {
        try
        {
            return await HandleCampaignTagPolling(request, input);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnCampaignTagAdded: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }

    [PollingEvent("On campaign message translation added or updated",
        Description = "Triggers when a campaign message is updated")]
    public async Task<PollingEventResponse<CampaignMessageMemory, PollingCampaignMessageContentResponse>>
        OnCampaignMessageContentUpdated(
            PollingEventRequest<CampaignMessageMemory> request,
            [PollingEventParameter] PollingCampaignMessageContentRequest input)
    {
        try
        {
            return await HandleCampaignMessageContentPolling(request, input);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnCampaignMessageContentUpdated: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }

    [PollingEvent("On email template tag added", Description = "Triggers when a email template tag is added")]
    public async Task<PollingEventResponse<DateMemory, PollingEmailTemplateResponse>> OnEmailTemplateTagAdded(
        PollingEventRequest<DateMemory> request,
        [PollingEventParameter] PollingEmailTemplateRequest input)
    {
        try
        {
            return await HandleEmailTemplateTagPolling(request, input);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnEmailTemplateTagAdded: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }
        ...
        [PollingEventParameter] PollingCanvasRequest input)
    {
        try
        {
            return HandleCanvasTagPolling(request, input);
        }
        catch (Exception e)
        {
            InvocationContext.Logger?.LogError($"BrazeOnCanvasTagAdded: {e.Message}; StackTrace: {e.StackTrace}", []);
            throw;
        }
    }
        ...
        PollingCampaignRequest input)
    {
        ...
        // Does it make sense to set it to max of all or max of updated only? This is a polling event that triggers every 5 minutes
